    ...
    try {
       ...
       try {
         for (const item of data) {
           ...
           await client.query(
             'INSERT INTO tally_entities(id, slug, name, proposals_count, active_proposals_count, token_holders_count, voters_count, metadata) VALUES($1, $2, $3, $4, $5, $6, $7, $8)',
             [
               item.id,
               item.slug,
               item.name,
           ...
           // Insert into chain_ids
           for (const chainId of item.chainIds) {
             await client.query(
               'INSERT INTO tally_chain_ids(entity_id, chain_id) VALUES($1, $2)',
               [item.id, chainId]
             );
           }
           console.log('Entry inserted successfully into tally_chain_ids');
   
           // Insert into governor_ids
           for (const governorId of item.governorIds) {
             await client.query(
               'INSERT INTO tally_governor_ids(entity_id, governor_id) VALUES($1, $2)',
               [item.id, governorId]
             );
           }
